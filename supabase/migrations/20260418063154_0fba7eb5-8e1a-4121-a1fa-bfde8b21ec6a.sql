-- Re-habilitar trigger de asignación automática
ALTER TABLE public.leads_interinos ENABLE TRIGGER trg_asignar_abogado_por_provincia;

-- Restaurar política con validaciones completas
...
-- Limpiar lead de prueba creado durante la verificación
DELETE FROM public.leads_interinos WHERE email = 'jsclient@test.local';